$(function(){
    ...
    const $visual = $(".visual");
    const $slides = $(".visual>.visual-container>li");
    const $indicator = $(".visual>.visual-pagination>li>a");

    const $btnPrev = $(".visual>.visual-prev");
    const $btnNext = $(".visual>.visual-next");

    const $btnAuto = $(".visual>.visual-auto");//자동재생 버튼
    ...
    $btnAuto.on('click', function(evt){
        ...
        if($(this).hasClass('pause')){//재생중 상태
            ...
            autoPlay();
            $visual.unbind('mousemove'); //이벤트 제거
        }
    });
